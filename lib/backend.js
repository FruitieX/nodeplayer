 ...
 * @param {Buffer} chunk - New song data since last call
...
Backend.prototype.encodeSong = function(stream, seek, song, callback) {
    ...
    var incompletePath = path.join(config.songCachePath, self.name, 'incomplete',
                                   song.songId + '.opus');
...
Backend.prototype.cancelPrepare = function(song) {
    this.log.error('FATAL: backend does not implement cancelPrepare()!');
    callback(new Error('FATAL: backend does not implement cancelPrepare()!'));
};
