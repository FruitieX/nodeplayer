...
function Player(options) {
    ...
    this.playlistPos    = options.playlistPos       || 0;
...
// - call prepareSongs()
Player.prototype.refreshPlaylist = function() {
...
Player.prototype.findSongPos = function(at) {
    var player = this;
    ...
    if (at === -1) {
        // value of -1 is beginning of queue
        listType = 'queue';
    } else if (!at) {
        // falsy value is beginning of playlist
        listType = 'playlist';
    } else {
        // other values assumed to be UUIDs: search playlist and queue
        listIndex = _.findIndex(player.playlist, function(song) {
            return song.uuid === at;
        });

        if (listIndex === -1) {
            // song was not found in playlist, search queue
            listType = 'queue';
            listIndex = _.findIndex(player.queue, function(song) {
                return song.uuid === at;
            });
        }
 ...
 *                      (null = start of playlist, -1 = start of queue)
 * @param {Object[]} songs - List of songs to insert
 */
Player.prototype.insertSongs = function(at, songs) {
    var player = this;

    var pos = player.findSongPos(at);

    // insert the songs
    var args = [pos.listIndex, 0].concat(songs);
    Array.prototype.splice.apply(player[pos.listType], args);

    // sync player state with changes
    if (pos.listType === 'playlist') {
        if (pos.listIndex <= player.playlistPos) {
            // songs inserted before playlist pos, add to it the no. of songs
            player.playlistPos += songs.length;
        } else {
            // songs inserted after playlist pos, check if next song changed
            if (pos.listIndex === player.playlistPos + 1 && !player.queue.length) {
                // inserted songs immediately after playlist pos and queue is empty,
                // next song must have changed so prepare it
                prepareSong(player.playlist[player.playlistPos + 1]);
            }
        }
    } else {
        // queue modified, only way the next song changed is if we inserted at
        // index zero
        if (pos.listIndex === 0) {
            prepareSong(player.queue[0]);
        }
    }
};

Player.prototype.getNowPlaying = function() {
    var song = this.playlist[this.playlistPos];
    return song ? song.uuid : undefined;
};
 ...
 */
Player.prototype.removeSongs = function(at, cnt) {
    var player = this;

    var pos = player.findSongPos(at);
    var oldPlaylistPos = player.playlistPos;
    var oldNowPlaying = player.getNowPlaying();

    var deleted = 0;

    // delete items before playlist pos
    if (pos < oldPlaylistPos) {
        deleted += player.playlist.splice(pos, Math.min(cnt, oldPlaylistPos - pos)).length;
        player.playlistPos -= deleted;
    }

    // delete now playing item
    if (pos + cnt >= oldPlaylistPos) {
        deleted += player.playlist.splice(pos, 1).length;
    }

    // delete items after old now playing item
    if (pos + cnt > oldPlaylistPos) {
        player.playlist.splice(pos, cnt - deleted);
    }

    // sync player state with changes
    if (pos.listType === 'queue') {
        // starting delete from queue, now playing can't change
        if (pos.listIndex === 0) {
            // but may need to prepare if we remove the first item
            prepareSong(player.queue[0] || player.playlist[oldPlaylistPos + 1]);
        }
    } else {
        if('np song changed') {
        }
...
Player.prototype.replacePlaylist = function(backendName, playlistId) {
    var player = this;
    ...
    if (backendName === 'core') {
        ...
        function(err, playlist) {
            if (err) {
                return player.logger.error('error while fetching playlist' + err);
            }
    ...
    if (!backend) {
        return player.logger.error('replacePlaylist(): unknown backend ' + backendName);
    }

    if (!backend.getPlaylist) {
        return player.logger.error('backend ' + backendName + ' does not support playlists');
    }

    backend.getPlaylist(playlistId, function(err, playlist) {
        if (err) {
            return this.logger.error('error while fetching playlist' + err);
        }
