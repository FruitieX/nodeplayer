...
Player.prototype.removeSongs = function(at, cnt) {
    ...
    if (pos < player.playlistPos) {
        deleted += player.playlist.splice(pos, Math.min(cnt, player.playlistPos - pos)).length;
    }

    // delete now playing item
    if (pos + cnt >= player.playlistPos) {
    ...
    if (pos.listType === 'queue') {
        ...
        if (pos.listIndex === 0) {
            ...
            prepareSong(player.queue[0] || player.playlist[player.playlistPos + 1]);
        }
    } else {
    }
};
