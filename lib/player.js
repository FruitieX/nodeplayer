...
function Player(options) {
    ...
    async.series([
        function(callback) {
            modules.loadBuiltinPlugins(player.callHooks, player.pluginVars, function(plugins) {
                player.plugins = plugins;
                player.callHooks('onBuiltinPluginsInitialized');
                callback();
            });
        }, function(callback) {
            modules.loadPlugins(config.plugins, player.pluginVars, forceUpdate, player.callHooks,
            function(results) {
                player.plugins = player.plugins.concat(results);
                player.callHooks('onPluginsInitialized');
                callback();
            });
        }, function(callback) {
            modules.loadBuiltinBackends(player.callHooks, function(backends) {
                player.backends = backends;
                player.callHooks('onBuiltinBackendsInitialized');
                callback();
            });
        }, function(callback) {
            modules.loadBackends(config.backends, forceUpdate, player.callHooks, function(results) {
                player.backends = player.backends.concat(results);
