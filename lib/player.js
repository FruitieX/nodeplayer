...
/**
 * Change to song
 * @param {String} uuid - UUID of song to change to, if not found in queue, now playing is removed
 */
Player.prototype.changeSong = function() {
};
...
Player.prototype.endOfSong = function() {
    ...
    if (this.queue.curQueuePos === this.queue.getLength() - 1) {
        // end of playlist
        this.queue.curQueuePos = -1;
    } else {
        this.queue.curQueuePos++;
    }
    ...
    this.queue.songEndTimeout = null;
    this.prepareSongs();
};
...
Player.prototype.prepareSongs = function() {
    ...
    async.series([
        ...
        function(callback) {
            ...
            var song = player.queue.songs[player.queue.curQueuePos + 1];
            if (song) {
                player.prepareSong(song, callback);
            } else {
                // bail out
