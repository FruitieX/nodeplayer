...
Player.prototype.endOfSong = function() {
    var player = this;
    var np = player.playlist[player.curPlaylistPos];

    if (player.curPlaylistPos === playlist.length - 1) {
        // end of playlist
...
Player.prototype.insertSongs = function(at, songs, callback) {
    ...
    if (player.curPlaylistPos === -1) {
        // playlist was empty, start playing first song
        player.curPlaylistPos++;
        player.prepareSongs();
    } else if (pos <= player.curPlaylistPos) {
