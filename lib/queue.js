...
Queue.prototype.insertSongs = function(at, songs) {
    ...
    songs = _.map(songs, function(song) {
        return new Song(song, this.backends[song.backendName]);
    }, this);

    // make sure there were no errors while creating Song objects
    var err = _.find(songs, function(song) {
        return song instanceof Error;
    });
    if (err) {
        return err;
    }
